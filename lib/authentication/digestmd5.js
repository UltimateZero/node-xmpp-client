...
function md5 (s, encoding) {
  ...
  return hash.update(s).digest(encoding)
}
function md5Hex (s) {
  return md5(s, 'hex')
}
...
DigestMD5.prototype.responseValue = function (s) {
  ...
  console.log("D:", dict)
  console.log('cnonce', this.cnonce)

  var value
  if (dict.nonce && dict.qop) {
    this.nonce_count++
    var y = md5(Buffer.concat([Buffer.from(this.authcid), Buffer.from(':'), Buffer.from(this.realm), Buffer.from(':'), 
     Buffer.from(this.password)]));
    var a1 = Buffer.concat([Buffer.from(y), Buffer.from(':'),
      Buffer.from(dict.nonce), Buffer.from(':'),
      Buffer.from(this.cnonce)])
    ...
    console.log('ha1', md5Hex(Buffer.from(a1)))

    value = md5Hex(md5Hex(Buffer.from(a1)) + ':' +
      dict.nonce + ':' +
      this.getNC() + ':' +
      this.cnonce + ':' +
      dict.qop + ':' +
